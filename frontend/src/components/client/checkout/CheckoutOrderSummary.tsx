...
export default function CheckoutOrderSummary({
  ...
  return (
    <aside className="lg:col-span-5 xl:col-span-4">
      {" "}
      {/* Đảm bảo class này khớp với layout cha */}
      <div className="sticky top-20 rounded-lg bg-white p-6 shadow-lg ring-1 ring-gray-200/50">
        <h2
          id="summary-heading"
          className="mb-4 border-b pb-4 text-xl font-semibold text-gray-900"
        >
          Tóm tắt đơn hàng
        </h2>
                      ...
                      {item.variantInfo?.options &&
                        item.variantInfo.options.length > 0 && (
                          <p className="mt-1 text-xs text-gray-500">
                            {item.variantInfo.options
                              .map(
                                (opt) => `${opt.attributeName}: ${opt.value}`,
                              )
                              .join(" / ")}
                          </p>
                        )}
            ...
            <div className="mt-6 space-y-3 border-t border-gray-200 pt-6">
              ...
              {appliedCouponFull && discount === 0 && (
                <div className="flex items-center rounded-md bg-yellow-50 p-2 text-xs text-yellow-600">
                  ...
                  <span>
                    Mã &quot;{appliedCouponFull.code}&quot; không đủ điều kiện
