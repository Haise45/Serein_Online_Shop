...
export default function CheckoutPageClient() {
  ...
  useEffect(() => {
    ...
    if (!fullCart || selectedCartItemIds.length === 0) {
      toast.error(t("noItemsToCheckoutToast"), { id: "checkout-no-selection" });
      router.replace("/cart");
      return;
    }
    ...
    if (itemsToCheckout.length !== selectedCartItemIds.length) {
      toast.error(t("staleDataToast"), { id: "checkout-stale-items" });
      router.replace("/cart");
      return;
    }
    ...
    if (stockErrorInCheckout) {
      toast.error(
        t("stockErrorToast", {
          name: stockErrorInCheckout.name,
          stock: stockErrorInCheckout.stock,
        }),
        { id: "checkout-stock-error" },
      );
      router.replace("/cart");
    }
  }, [
  ...
  if (itemsToCheckout.length === 0) {
    ...
    return (
      <div className="flex min-h-[400px] flex-col items-center justify-center space-y-4">
        <FiLoader className="h-12 w-12 animate-spin text-indigo-600" />
        <p className="text-lg text-gray-700">{t("checkingCart")}</p>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 gap-x-12 gap-y-10 lg:grid-cols-12">
        ...
        <div className="lg:col-span-5 xl:col-span-4">
          <CheckoutOrderSummary
            ...
            stockErrorObject={stockErrorInCheckout}
