...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const t = await getTranslations({
    locale,
    namespace: "Metadata.orderDetails",
  });

  const orderIdSuffix = orderId.toString().slice(-6).toUpperCase();

  return {
    title: t("title", { id: orderIdSuffix }),
    description: t("description", { id: orderIdSuffix }),
  };
}
...
export default async function UserOrderDetailPageContainer({ params }: Props) {
